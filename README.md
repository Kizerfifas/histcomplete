...
|----------|---------|----------|
| Дополнение из истории | **Alt+h** | Подставить команду из истории по началу набранной строки |
| Список вариантов | **Alt+h** дважды | Показать совпадения; второе нажатие подставляет самую свежую |
...
2. Нажимаете **Alt+h**
3. Строка заменяется на полную команду из истории, например:  
   `ls -la ~/projects`
4. При необходимости правите и нажимаете **Enter**

Если совпадений несколько:

- первое **Alt+h** — дописывает общий префикс или подставляет самый свежий вариант;
- второе **Alt+h** — печатает нумерованный список в stderr и подставляет первый (самый свежий) вариант;
- **↑** / **↓** — перебор всех команд, начинающихся с `ls`.
...
ls               # набрать
# Alt+h          # подставить, например: ls -la ~/projects
```
...
git              # набрать
# Alt+h          # → git status, git log --oneline -5, ...
```
